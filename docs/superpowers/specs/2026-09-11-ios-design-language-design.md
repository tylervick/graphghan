...
The PWA icon's composition without the standing stones:
...
plain composition). The 1024 pt master is generated by `ios/Scripts/make_icon.py` (`mise run
icon` from `ios/`), with every value expressed as a fraction of the side so `site/build.py`'s
`make_icons` can adopt the same geometry later (section 11). The icon has no alpha; iOS applies
its own mask.
