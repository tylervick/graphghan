...
The PWA icon with the weave added, on the site's 16-unit grid (1 unit = 6.25% of the side):

- Moss sky with the weave (5% Cream); a Moss Deep hill band from 12 units down.
- A Gold (#D9A21B) moon centered at (12.75, 4.25) units, radius 1.75.
- Three Charcoal (#2B2F33) standing stones on the horizon: left edges at 3.5, 6.5, and 9.5
  units, 1.8 units wide, 5, 7, and 5.5 units tall.

A crocheted chain along the horizon was drawn and set aside on 2026-09-11 (Tyler preferred the
PWA's stones). The 1024 pt master is generated by `ios/Scripts/make_icon.py` (`mise run icon`
from `ios/`); `site/build.py`'s `make_icons` draws the same geometry without the weave. The icon
has no alpha; iOS applies its own mask.
