...
- Strip: rows around the current one drawn from the grid in a Panel box with a Line border and
  14 pt radius; the current row outlined 2 pt Heather; rows already worked at 45% opacity; a
  Heather marker on the starting edge.
- Chips: section 6.3.
...
- Done field: everything below the card. "Done" in the Done style centered. Tapping anywhere in
  the field advances. The Back rail sits on the leading edge of the field, 84 pt wide, Moss Deep,
  top trailing corner 22 pt, with the return arrow and "Back" in Label at 85% Cream. Swipe right
  anywhere still goes back.
- Finished: the card holds the strip and a cream panel with "Finished" (Literata 40) and the
  send-off line; the field reads "Close" and dismisses.
...
Primary: Moss fill, Cream text in Body bold, pill, 50 pt tall. Secondary: Panel fill, Line
border, Moss text. Text buttons in Moss; destructive text in the system red inside sheets and
...
and "Row 42 of 184" on one line; the current run as a 72 × 60 swatch with the count in
SF Rounded heavy 34 beside the name (New York 22) and the next line (SF Pro 14); Back as a 56 pt
circle at 12% white; Done as a Moss pill, 48 pt tall. The Dynamic Island keeps its current
layout with the swatch and count. Cream text throughout.
...
The 1024 pt master is generated by a script under `ios/` sharing its geometry with
`site/build.py`'s `make_icons`, so the app and the PWA can be brought back into step later. The
...
- A unit test that every yarn-colored surface obtains foreground and hairline from the one
  helper, with a cream-on-cream case.
