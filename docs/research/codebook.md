...
the field does not apply to this pattern type. Never infer: if the pattern does not say it, code
`unstated`. Quote the source text for any field marked **quote** so a second reader can check it.
...
| `object` | `blanket`, `washcloth`, `garment`, `accessory`, `toy`, `bag`, `wall-hanging`, `other` |
...
| `special_stitch_names` | semicolon-separated names, or `none` |
...
| Field | Values |
|---|---|
| `stitches_used` | semicolon-separated CYC abbreviations, US terms, e.g. `sc;hdc;dc` |
...
| `turning_chain_by_stitch` | e.g. `sc=1;dc=2` when the pattern chains differently before different rows; else `single` |
...
| `foundation_form` | `chain-count`, `chain-multiple` (e.g. "multiple of 6 + 1"), `fsc`, `magic-ring`, `other`, `n/a`, `unstated` — **quote** |
...
| `stitch_counts_given` | `every-row`, `changes-only`, `none` |
...
| `written_also` | `yes`, `no` — full written instructions alongside a chart |
| `rs_ws_stated` | `yes`, `no` |
