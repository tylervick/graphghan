...
| 8 | Written instructions must accompany a chart (accessibility) | settled for design | C (34 charted): 13/34 ship written rows — the standard is not the practice. Ravelry has a "Universal – no written language" class. S: BANA guidelines; Atherley ("charts can be used, but full written instructions must also appear"); T: Stitchmastery and Crochetpop both emit written rows with foundation and turning chain | — |
