...
@Suite struct WorkFeedbackTests {
    ...
    @Test func runWithinRow() {
        ...
        #expect(WorkFeedbackRule.feedback(for: step, in: Self.seq) == .run)   // Gd: row 1 has no previous row → plain run
    }

    @Test func rowBoundaryAndNewColor() {
        let toRow2 = WorkEngine.apply(.advance, to: Cursor(row: 1, run: 2), in: Self.seq)!
        #expect(WorkFeedbackRule.feedback(for: toRow2, in: Self.seq) == .row)  // Gd was used in row 1
        let toY = WorkEngine.apply(.advance, to: Cursor(row: 2, run: 1), in: Self.seq)!
        #expect(WorkFeedbackRule.feedback(for: toY, in: Self.seq) == .newColor)  // Y not in row 1
    }
