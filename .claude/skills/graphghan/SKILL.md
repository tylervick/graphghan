...
| `graphghan import <file> --into <slug> [--palette toml] [--page N --region K] [--rows written\|grid] [--reader apple] [--dry-run]` | a chart PDF, a picture of a chart, a 1-px PNG, OXS, or CSV into a pattern folder; lists every grid it finds; `--reader apple` tries the on-device model instead of you (macOS 27) |
